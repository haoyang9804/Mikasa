...
public:
  ...
  void update(const T &ul, const T &ur, const NT &u_val) {
    ASSERT(ul >= root_p->l && ur <= root_p->r,
           "lower bound or upper bound is out of range");
  ...
  NT query(const T &ql, const T &qr) {
    ASSERT(ql >= root_p->l && qr <= root_p->r,
           "lower bound or upper bound is out of range");
    ASSERT(updateF_ele || updateF_seg,
           "You should define your update function.");
    STNode<T, NT> *cur_node_p = this->root_p;
...
private:
  ...
  inline T cut_it_half(T l, T r) { return (l + r) / 2; }
  ...
  void spread_laziness(STNode<T, NT> *cur_node_p) {
    if (cur_node_p->is_lazy() && !cur_node_p->is_leaf()) {
      ...
      cur_node_p->get_left()->set_val(updateF_seg(
          cur_node_p->get_left()->get_val(), cur_node_p->get_lazy_val(),
          cur_node_p->get_left()->l, cur_node_p->get_left()->r));
      cur_node_p->get_right()->set_val(updateF_seg(
          cur_node_p->get_right()->get_val(), cur_node_p->get_lazy_val(),
          cur_node_p->get_right()->l, cur_node_p->get_right()->r));
      cur_node_p->lazy_off();
    }
  }
